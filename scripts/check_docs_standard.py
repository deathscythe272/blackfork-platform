...
docs/DOCS-STANDARD.md, and its diagrams must obey the mechanical diagram rules:
left to right, at most seven nodes, every node glossed. The one named exemption is
...
from lane1_rules import diagram_rules  # noqa: E402
...
for path in sorted(pathlib.Path("docs").rglob("*.md")):
    ...
    for f in diagram_rules(text, path.as_posix()):
