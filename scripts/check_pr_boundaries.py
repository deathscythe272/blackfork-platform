...
an eval case (R6), a trust-boundary change ships with a threat-model change (R9), and no
secret is written out in the diff (R10; a reference to one by name is not a secret), in
...
from lane1_rules import added_lines_from_patch, boundary_rule, removed_lines_from_patch, secret_rule, tool_rule  # noqa: E402
...
fails = tool_rule(paths, added) + boundary_rule(paths, added, removed) + secret_rule(added)
for f in fails:
