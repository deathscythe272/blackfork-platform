...
```mermaid
flowchart LR
  PRIME["DoD prime contractor<br><i>contract pushes compliance rules down</i>"] --> SEC["Security team<br><i>three people, cannot hire</i>"]
  SOURCES["Cloud and scanners<br><i>logs, scans, sensor alerts</i>"] --> PROV["PROVENANCE<br><i>continuous, cited audit evidence</i>"]
  SEC --> PROV
  PROV --> SIGN["Human sign-off<br><i>the team approves each packet</i>"]
