...
| Tampering | Terraform records altered or deleted so the next apply does the wrong thing | Versioned bucket with public access prevented at the bucket; only the two deployers and the operator may write | T1-CI-03: versioning and public-access prevention verified on the bucket | Planned, phase 6 step 1 (bootstrap apply pending) |
...
| T1-CI-01 to T1-CI-04 | B8 | `infra/modules/delivery-plane/`; phase 6 steps 1 and 2 | Planned |

Count: 39 threat rows, 18 passing, 1 measured with mitigation upstream, 15 planned with a phase, 5 gaps named. The gaps are
