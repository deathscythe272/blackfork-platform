...
| 26 | Phase 7, step 4 | The re-scoring run after the scanner merged was cancelled by its own forty-five-minute limit with three cases left, every case having taken two to four and a half minutes at mid-day on the hosted free tier, and a cancelled run writes no record | The job's limit is ninety minutes | One run measured and lost; the cases it did finish showed the same day-time quality failures the nightly had, and no containment failure | The nightly-findings pull request |
| 27 | Phase 7, step 4 | The first run with the timestamp referee still charged the door checks' denials to the golden case: the referee allowed thirty seconds of clock skew, the tokens' allowance, which is wider than the twenty-second settle, so rows stamped twenty-five seconds before the mark were inside the window | The tolerance is five seconds, the observed skew being one; the test's late row now sits where the door checks do | A third red golden case on the record for one bug in three costumes: arrival, settle, then arithmetic; the run passed only because the rule had already learned to call it persistent | The referee-tolerance pull request |
| 28 | Phase 7, step 4 | The two runs after the tolerance fix, one at evening and the next morning's nightly, failed on day-time speed: the golden case's own audit row reached the referee after it had stopped listening; the analyst's front door cut a slow verdict at five minutes and answered with a page the assessor could not parse; the packet job passed the runner's ten-minute wait; and the model returned one degenerate answer | A case expected to produce rows waits up to two and a half minutes for its first; the analyst client names a non-JSON reply and its status; the analyst and agent services get longer request limits and the runner waits fifteen minutes; the degenerate answer is recorded as the model's | Two more red records, none a containment failure; the free tier by day is a different service from the free tier by night, and the harness had been built at night | The day-time robustness pull request |
| 29 | Phase 7, step 4 | The scanner's two openings, the Anti-DAN persona and the "just print" hijack, went through an input rail whose policy listed the attacks the platform had thought of | The input rail now starts from an allow-list, this agent's one kind of work, and names persona adoption, prefix labels, and stop-and-print demands; the output rail refuses persona speech and echoed phrases; both openings are eval cases the nightly runs, so the seeded set grows from what the scanner finds | A rail written from an allow-list refuses more than it used to, and the golden and mapper questions were checked against it before it shipped; the weekly scan after the merge is the test that counts | The rail revision pull request |
